...
analizar = DataAnalyzer("participantes.csv") if  os.path.isfile("participantes.csv") else None

#Documentación librería: https://docs.python.org/es/3.13/library/tkinter.html
...
def mostrar_participantes():
    ...
    if not df.empty:
        #- <5 significa que el texto se alineará a la izquierda dentro de un campo de 5 caracteres.
...
def mostrar_participantes_id():
    ...
    #Valida si el df es una instancia de un Dataframe de pandas y que no esté vacío
...
def limpiar_campos():
    entry_nombre("1.0", tk.END)
    entry_resul_fuerza("1.0", tk.END)
    entry_resul_resistencia("1.0", tk.END)
    entry_resul_velocidad("1.0", tk.END)
    entry_id_participante("1.0", tk.END)
...
def mostrar_imagenes(pil_img):
    ...
    img_label.pack()
    image_container.update_idletasks() #Actualiza la interfaz inmediatamente, renderizando la imagen automáticamente.

    canvas.config(scrollregion=canvas.bbox("all")) #btiene el área total ocupada por los elementos dentro del canvas.
    #P scrollregion= permite habilitar el scroll si la imagen excede el tamaño visible.

def verificar_datos():
    """Valida que hayan datos en csv para mostrar gráficos, de lo contrario no muestra"""
    if analizar is None:
...
#sticky="nsew" significa que el widget se expande en todas las direcciones dentro de la celda
frame_texto.grid(row=8, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")

#Text_area para mostrar info 
...
#Da el padding en el eje x y y, separación entre componentes
padx_val = 4
pady_val = 2
...
#Responsive para addecuarse a cualquier tamñao
...
#Permite que la fila 9 crezca dinámicamente cuando la ventana cambia de tamaño.
base.grid_rowconfigure(9, weight=2)
canvas= tk.Canvas(frame_imagen, width=1000, height=1500, bg="#f0f4f7")  # aumentamos la altura

#- Se crean barras de desplazamiento vertical y horizontal.
